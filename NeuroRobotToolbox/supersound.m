...
for nstep = 1:nsteps
    ...
    xtones = randsample(length(these_tones), 1 + round(4 * rand));
    xtones = sort(xtones);
    xtones(diff(xtones) < 4) = [];
    xtones2 = randsample(length(these_tones2), round(0 * rand));
    ...
    these_tones = these_tones(xtones);
