...
%     end
    ...
    elseif ~isnan(sdata(ntuple, 1))
        rl_state = sdata(ntuple, 1);
    elseif ~isnan(sdata(ntuple, 2))
        rl_state = sdata(ntuple, 2);
    end

    if sdata(ntuple, 3) == sdata(ntuple, 4)
        rl_next_state = sdata(ntuple, 3);
    elseif ~isnan(sdata(ntuple, 3))
        rl_next_state = sdata(ntuple, 3);
    elseif ~isnan(sdata(ntuple, 4))
        rl_next_state = sdata(ntuple, 4);
    end
