        ...
        for p2 = 1:nneurons
            ...
            if w ~= 0
                ...
                if draw_synapse_strengths && ~small_neurons && ~bg_neurons(p2)
                    w = round(w);
                    plot_neuron_synapses(p1, p2, 3) = text(x2, y2 + 0.15, num2str(w), 'fontsize', bfsize + 2, 'verticalalignment', 'middle', 'horizontalalignment', 'center', 'fontname', gui_font_name, 'fontweight', gui_font_weight, 'color', [0.4 0.2 0]);
                end
            end
        end
    end
...
%                     plot_contact_synapses(nneuron, ncontact, 3) = plot(x2b, y2b, 'marker', 'd', 'markerfacecolor', [0.8 0.8 0.8], 'markeredgecolor', [0.8 0.8 0.8], 'markersize', fs);
                    plot_contact_synapses(nneuron, ncontact, 4) = text(x2b, y2b, char(vis_pref_names(this_vis_pref)), 'fontsize', bfsize + 2, 'verticalalignment', 'middle', 'horizontalalignment', 'center', 'FontWeight', 'bold'); % Seems to give error when drawing object detecting brain without use_cnn
                end
        ...
        for nneuron = 1:nneurons
            if neuron_scripts(nneuron)
                if neuron_scripts(nneuron) == 5
                    this_str = horzcat(state_net_name, ' ', rl_type, ' ', action_net_name);
                else
                    this_str = script_strs(neuron_scripts(nneuron)).name;
                end
                neuron_annotation(nneuron, 1) = text(neuron_xys(nneuron,1), neuron_xys(nneuron,2) - 0.2, num2str(this_str), 'fontsize', bfsize + 2, 'verticalalignment', 'middle', 'horizontalalignment', 'center', 'fontname', gui_font_name, 'fontweight', gui_font_weight);
            end
        end
    end
end
