...
button_play_audio = uicontrol('Style', 'pushbutton', 'String', 'Play audio', 'units', 'normalized', 'position', [0.7 0.05 0.2 0.05]);
set(button_play_audio, 'Callback', 'flag_play_audio = 1;', 'FontSize', 18)
...
while rak_cam.isRunning() && flag_run
    
    startTimings = [startTimings; clock];
    
    % Video stream
    imageMat = rak_cam.readVideo();
    ...
    % Write serial
    rak_cam.writeSerial('d:0;');
    rak_cam.writeSerial(sprintf('d:%d11;', ledOrder(ledCounter)));
...
%     if mod(serialCounter - 10, 500) == 0
    if flag_play_audio ~= 0
%         t = 0 : 1/1000 : 5;
%         y = sin(6.28 * 8 * t);
%         y = [y y y y]';
%         rak_cam.sendAudio2(y);
%         rak_cam.sendAudio('NeuroRobot_framework/EXPLOSION.mp3');
        flag_play_audio = 0;
%         rak_cam.sendAudio('Sounds/Car+Alarm.mp3');
        rak_cam.sendAudio('Sounds/wolf.mp3');
    end
    sendAudioTimings = [sendAudioTimings; clock];
    
    % Receive serial
    serialData = [serialData rak_cam.readSerial()];
