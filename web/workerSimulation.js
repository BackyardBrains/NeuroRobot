...
// per second it can send ±750 commands,
// we need to process every X commands so it can Produce Y commands per seconds.
...
self.onmessage = async function( eventFromMain ) {
    ...
    if (eventFromMain.data.message === 'STOP_THREAD_PROCESS') {
        ...
        try{
            ...
            console.log("Remove Function", err);
        }
    ...
    if (eventFromMain.data.message === 'INITIALIZE_WORKER') {
        ...
        // console.log("Module.webSimulationCallback");
        const url = 1;
        /* Add Function Error */
        // const callback = (pData) => {
        //     console.log("pData");
        //     console.log(pData);

        //     return 1;
        // };
        // pCallback = await Module.addFunction(callbackToWebFromSimulation, 'viiiii');
        // console.log("pCallback");
        // console.log(pCallback);
        //     Module.ccall('invokeCallback', null, ['number'], [pCallback]);
        /* Add Function Error */
        ...
        console.log("ptrStateBuffer: ", ptrStateBuffer);
...
let limitCommandPerSecond = 0;

function callbackToWebFromSimulation(rawPosState, messageRaw, messageLength, neuronSize, ptrPeriodicNeuronSpikingFlags) {
// function callbackToWebFromSimulation(rawPosState, message, rawPosCmdLen, ptrMotorCommandMessage, neuronSize, ptrPeriodicNeuronSpikingFlags){

    const command = HEAPU8.subarray(messageRaw, messageRaw + messageLength);
    // const message = String.fromCharCode(...command);
    const message = String.fromCharCode(...command);

    const posNotify = rawPosState >> 2;
    if (benchmarkSpeed == -1) {
        limitCommandPerSecond++;
        ...
        if (endTime - startTime > 1000) {
            console.log("curCommandPerSecond callback 5 Neurons : ", limitCommandPerSecond, endTime - startTime);
            commandPerSecond = Math.floor(limitCommandPerSecond / 20);
            benchmarkSpeed = 1;
        }
    }
    curCommandPerSecond++;
    if (curCommandPerSecond == commandPerSecond) {
        ...
        // sabStateBuffer[STATE.COMMAND_MOTORS] = (sabStateBuffer[STATE.COMMAND_MOTORS] + 1) % 3000;
...
function updateMotorCommand(rawPosState, message, rawPosCmdLen, ptrMotorCommandMessage, neuronSize, ptrPeriodicNeuronSpikingFlags){
    ...
    if (benchmarkSpeed == -1) {
        ...
        endTime = (new Date()).getTime();
        if (endTime - startTime > 1000) {
            // console.log("updateMotorCommand : ", limitCommandPerSecond, endTime - startTime);
            commandPerSecond = Math.floor(limitCommandPerSecond / 20);
            benchmarkSpeed = 1;
        }
    }
    ...
    curCommandPerSecond++;
    if (curCommandPerSecond >= commandPerSecond) {
        // console.log("Average curCommandPerSecond EM_ASM : ", bufferSimulationMessage.length);
        let infoStatusMax = [0, 0, 0, 0, 0, 0, 0];
        ...
        try {
            ...
            // console.log("Diode String : ", diodeString);
            ...
            // console.log("NOTIFY: ", rawPosState);
            if (sabStateBuffer[STATE.WEB_SOCKET] == 1) {
